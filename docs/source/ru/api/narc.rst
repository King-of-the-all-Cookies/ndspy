...
``ndspy.narc``: архивы NARC
============================

.. py:module:: ndspy.narc

Модуль :py:mod:`ndspy.narc` позволяет открывать и сохранять архивы *NARC*.

Архивы *NARC* устроены похоже на ROM: каждый файл можно адресовать по ID или по имени
из таблицы имён. Таблицы работают так же, как в ROM.

.. warning::

    По умолчанию :py:attr:`NARC.endiannessOfBeginning` устанавливается в
    ``'<'`` при создании нового :py:class:`NARC`. Однако некоторые инструменты,
    такие как `Tinke <https://github.com/pleonex/tinke>`_ и
    `narchive <https://github.com/nickworonekin/narchive>`_, не работают с
    файлами этого типа. Если вы сталкиваетесь с проблемами или игра
    неправильно загружает ваш *NARC*, попробуйте поставить ``'>'``.
