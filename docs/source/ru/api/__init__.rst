...
Пространство имён ``ndspy`` содержит функции и классы, которым не требуется
отдельный модуль.
...
.. py:data:: VERSION
    ...
    .. note::
        ...
        Это было добавлено в ndspy 2.0.0, поэтому в версиях 1.0.0 и 1.0.1 его
        нет. Тем не менее 2.0.0 вышел менее чем через неделю после 1.0.0, так
        что вполне безопасно предположить, что в современной версии оно существует.
