...
``ndspy.bmg``: BMG (сообщения)
===============================

.. py:module:: ndspy.bmg

Модуль :py:mod:`ndspy.bmg` поддерживает загрузку и сохранение файлов *BMG*.

В играх, где используются такие файлы, *BMG* обычно содержит большую часть
текста, отображаемого игроку (кроме текста, записанного в изображения). Игра
может иметь один или несколько *BMG* на каждый язык, и будет подгружать нужный
в зависимости от текущей языковой настройки. Каждое «сообщение» доступно по
индексу.

Некоторые *BMG*, особенно в играх Zelda для DS, содержат не только текст, но и
скрипты, управляющие прогрессом. ndspy умеет читать и сохранять блоки *FLW1*
и *FLI1*, которые др содержат, но декодирование и кодирование самих команд
очень специфично для отдельных игр и выходит за рамки ndspy.


Примеры
--------

Загрузите *BMG* из ROM и изучите сообщения и скрипты:
...
Загрузите *BMG* из файла, отредактируйте сообщение и сохраните результат обратно
в ROM:
...
Создайте новый *BMG* с кодировкой ``cp1252`` и сохраните его в файл:
